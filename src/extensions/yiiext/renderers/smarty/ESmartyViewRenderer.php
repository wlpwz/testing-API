...
class ESmartyViewRenderer extends CApplicationComponent implements IViewRenderer
{
	...
	public function renderFile($context,$sourceFile,$data,$return) {
        ...
        $error_reporting=error_reporting(E_ALL ^ E_DEPRECATED ^ E_STRICT ^ E_NOTICE);
		$data['this'] = $context;
		...
		if(isset(Yii::app()->setting)){
            $data['setting']=Yii::app()->setting;
		}
		if(isset(Yii::app()->user)){
            $data['user']=Yii::app()->user;
		}
		//Joe fix, this is very important
		$this->smarty->template_dir = dirname($sourceFile);
		//add template dir for template extends by xuetongqin 
		$this->smarty->addTemplateDir(Yii::app()->getBasePath() . DIRECTORY_SEPARATOR . "views/");
		$prefix=$sourceFile;
		$viewPath=Yii::app()->getViewPath();
		if(strpos($prefix, $viewPath)===0){
			$prefix=substr($prefix, strlen($viewPath));
		}
		$prefix=str_replace(array('/', '\\'), '|', $prefix);
		$template = $this->smarty->createTemplate($sourceFile, null, $prefix, $data, false);
		//render or return
		if($return){
