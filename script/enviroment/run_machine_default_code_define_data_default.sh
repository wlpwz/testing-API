...
REMOTE_RUN_MACHINE_DEFAULT_CODE_DEFINE_DATA_DEFAULT="${REMOTE_BASE_PATH}/script/remote_run_machine_default_code_define_data_default.sh"
...
function run() {
    ...
    spawn ssh ${machine} sh  ${REMOTE_RUN_MACHINE_DEFAULT_CODE_DEFINE_DATA_DEFAULT} -n ${define_code_new_code_ftp_path} -o ${define_code_old_code_ftp_path} -t ${platform_data_type} -d ${platform_data_num} -i ${task_id} -p ${ec_type} -A ${newolddiff} -B ${newdiff} -C ${olddiff} -D ${memory} -E ${speed} -F ${valgrind}
    expect {
...
function Usage() {
    local name=$1
    echo "Usage: $name  -n <define_code_new_code_ftp_path> -o <define_code_old_code_ftp_path> -t <platform_data_type> -d <platform_data_num> -i <task_id> -p <ec_type>-A <newolddiff> -B <newdiff> -C <olddiff> -D <memory> -E <speed> -F <valgrind>   [-h]"
...
if [ ${define_code_new_code_ftp_path}"x" == "x" -o ${define_code_old_code_ftp_path}"x" == "x" -o ${platform_data_type}"x" == "x" -o ${platform_data_num}"x" == "x" -o ${task_id}"x" == "x" -o ${ec_type}"x" == "x" -o ${newolddiff}"x" == "x" -o ${newdiff}"x" == "x" -o ${olddiff}"x" == "x" -o ${memory}"x" == "x" -o ${speed}"x" == "x" -o ${valgrind}"x" == "x" ];then
    Usage $0
    exit 1
fi
