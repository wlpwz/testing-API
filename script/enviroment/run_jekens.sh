...
REMOTE_RUN_JEKENS="${REMOTE_BASE_PATH}/script/remote_run_jekens.sh"
...
function run() { 
    ...
    spawn ssh ${machine} sh   ${REMOTE_RUN_JEKENS} -n $define_code_new_code_ftp_path -o $define_code_old_code_ftp_path -t $task_id -p ${ec_type} -A $newolddiff -B $newdiff -C $olddiff -D $memory -E $speed -F $valgrind -c $covfile
    expect {
...
function update_mysql() { 
    ...
    local result=$1
    if [ $result -eq 0 ];then
        echo -e "use project_ktv; update JekensRun set STATUS='done',RUN_RESULT='ftp://$HOSTNAME:${REMOTE_BASE_PATH}/workspace/jekens/${task_id}/result.tar.gz' where TASK_ID=${task_id}" | $MYSQL -h${HOST} -P${PORT} -u${USER} -p${PASSWORD} 
        exit_if_error "use project_ktv; update JekensRun set STATUS='done',RUN_RESULT='ftp://$HOSTNAME:${REMOTE_BASE_PATH}/workspace/jekens/${task_id}/result.tar.gz' where TASK_ID=${task_id} failed" 
    elif [ $result -eq 1 ];then
        echo -e "use project_ktv; update JekensRun set STATUS='fail' where TASK_ID=${task_id}" | $MYSQL -h${HOST} -P${PORT} -u${USER} -p${PASSWORD} 
        exit_if_error "use project_ktv; update JekensRun set STATUS='fail' where TASK_ID=${task_id} failed" 
    fi
}
