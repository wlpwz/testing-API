...
cat $1 | awk '{ ex1 = index("mg", substr($4, length($4))); if (ex1 == -1) {ex1 = 0}; val1 = substr($4, 0, length($4) - 1); prod1 = val1 * 1024^(ex1); ex2 = index("mg", substr($5, length($5))); if (ex2 == -1) {ex2 = 0};val2 = substr($5, 0, length($5) - 1); prod2 = val2 * 1024^(ex2); print $1, $2, $3, prod1, prod2, $6, $7, $8, $9}' > tmp && mv tmp $1
...
sed -i '1 i\date time pid virt res shr cpu mem command' $1
